...
browser: ['Jadi Bot By Atak', "Chrome", "1.0.0"],
...
try {
conn = chatUpdate.messages[0]
if (!conn.message) return
conn.message = (Object.keys(conn.message)[0] === 'ephemeralMessage') ? conn.message.ephemeralMessage.message : conn.message
if (conn.key && conn.key.remoteJid === 'status@broadcast') return
if (!conn.public && !conn.key.fromMe && chatUpdate.type === 'notify') return
if (conn.key.id.startsWith('BAE5') && conn.key.id.length === 16) return
m = smsg(conn, msg, store)
require('./Fardan')(conn, m, chatUpdate, store)
} catch (err) {
console.log(err)}
})
...
conn.sendMessage('6287721317870@s.whatsapp.net', {text: txt, mentions : [user]})
}
