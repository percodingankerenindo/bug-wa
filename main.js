/**
   * Create By Dika Fardan yo
   * Contact Me on wa.me/6285696481452
...
const { smsg, formatp, formatDate, getTime, isUrl, sleep, clockString, runtime, fetchJson, getBuffer, jsonformat, format, parseMention, tanggal, getRandom, getGroupAdmins } = require('./lib/myfunc')
    ...
    hisoka.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                name = await hisoka.getName(num)
                    teks = `Hai Kak @${num.split("@")[0]} Selamat Datang Di Group *${metadata.subject}* \nCongrats!! 🎉\n\n*Create Session*\nOfficial Group : https://bit.ly/3EAhiyQ\n@6281523940672\nSupport @0`
                    let mok = [
{"buttonId": `owner`,"buttonText": {"displayText": `Delevor`},"type": "RESPONSE"},
{"buttonId": `menu`,"buttonText": {"displayText": `Menu Bot`},"type": "RESPONSE"}]
let options1 =
{ forward,
externalAdReply: {
...
thumbnail: thumb,
sourceUrl: `https://chat.whatsapp.com/GLumPCuWpd3Gk6vuw3vpvk`
}
}
hisoka.sendButDoc(anu.id, `Welcome ${name} Selamat Datang Di ${metadata.subject}`, 'Jangan Lupa Baca deskripsi yah kak',thumb, mok, options1, )
                    hisoka.sendMessage(num, {text: teks, mentions:parseMention(teks)})
                } else if (anu.action == 'remove') {
                    teks = `Yah Kak nya Out Dari ${metadata.subject}\n Happy Trus Yah Kak *Congrats!!* 🎉\nOfficial Group: _https://bit.ly/3EAhiyQ_ `
                    hisoka.sendMessage(num, {text: teks, mentions:parseMention(teks)})
    ...
    * 
     ...
     * @param {*} options 
     */
     hisoka.sendButDoc = async(id, text1, desc1, gam1, but = [], options,  options1 = {}) => {	
    var AppType = "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
    const buttonMessage = {
    contextInfo: options,
    document: fs.readFileSync('./lib/file.docx'),
    mimetype: AppType, 
    title : "Footer text", 
    ...
    return await hisoka.sendMessage(id, buttonMessage,options1)
    } 
